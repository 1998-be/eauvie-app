...
def conseil_classe(cl):
    return ['Cette eau est conforme aux normes OMS. Elle peut être consommée sans traitement préalable. Veillez à maintenir des conditions de stockage hygiéniques.',
            ...
            'DANGER EXTRÊME. Cette eau présente un risque sanitaire majeur. Tout contact doit être évité. Signalez immédiatement la situation aux autorités sanitaires compétentes.'][cl]
...
def generer_pdf(mesures, classe, probabilites, analyste='', lieu='', source=''):
    ...
    conductivite=mesures['cond']; nitrates=mesures['no3']; Dioxygène=mesures['Dioxygène']; ecoli=mesures['ecoli']
    ...
    story=[]
    header=[[Paragraph('<b>💧 EauVie</b>',S('hx',fontName='Helvetica-Bold',fontSize=24,textColor=BLANC,alignment=TA_CENTER))],[Paragraph("Analyse intelligente de la qualité de l’eau — Normes OMS<br/>afin de garantir une consommation rassurante et bénéfique.",S('hx2',fontName='Helvetica',fontSize=10.5,textColor=colors.HexColor('#d0eeff'),alignment=TA_CENTER,leading=16))],[Paragraph('Proposée par Charles MEDEZOUNDJI',S('hx3',fontName='Helvetica-Oblique',fontSize=9,textColor=colors.HexColor('#a8d8ff'),alignment=TA_CENTER))],]
    ...
    story.append(Paragraph("L’eau, ressource vitale et irremplaçable, est au cœur d’une crise sanitaire silencieuse qui ravage le continent africain. Plus de <b>400 millions d’Africains</b> n’ont toujours pas accès à une eau potable sûre et durable (OMS/UNICEF, 2025). En Afrique subsaharienne, des milliers de vies sont perdues chaque jour — principalement des enfants de moins de cinq ans — victimes de maladies diarrhéiques, du choléra, de la fièvre typhoïde et d’autres pathologies directement liées à la consommation d’une eau de mauvaise qualité.",sb))
    ...
    story.append(Paragraph("Sur la base des sept mesures physico-chimiques et microbiologiques et de l’analyse par l’algorithme Random Forest, l’échantillon d’eau analysé présente le profil suivant :",sb))
    ...
    if classe>0:
        ...
        methodes=[('1. Ébullition',"Porter l’eau à ébullition pendant au moins 5 minutes. Laisser refroidir dans un récipient propre et couvert. Efficace contre les bactéries, les virus et les parasites."),('2. Filtration artisanale',"Couches successives : gravier grossier, gravier fin, sable grossier, sable fin, charbon de bois actif. À combiner obligatoirement avec l’ébullition."),('3. SODIS solaire',"Bouteilles en plastique transparent exposées 6 heures au soleil (ciel clair) ou 2 jours (nuageux). Méthode gratuite, validée par l’OMS."),('4. Chloration',"2 gouttes d’eau de Javel à 5 % par litre d’eau trouble (1 goutte si claire). Attendre 30 minutes avant de consommer."),('5. Moringa oleifera',"Broyer 2-3 graines sèches en poudre fine. Ajouter à 1 litre d’eau turbide, agiter 1 min puis 5 min lentement. Décanter 1 heure et compléter par ébullition.")]
...
with st.expander('📋 Normes OMS de référence — 7 paramètres'):
    st.markdown('| Paramètre | ✅ Potable | ⚠️ Douteuse | ❌ Polluée | ☠️ Dangereuse |')
    st.markdown('|---|---|---|---|---|')
    st.markdown('| pH | 6,5 à 8,5 | 5,5 à 9,0 | 4,5 à 5,5 | inf. à 4,5 |')
    st.markdown('| Turbidité (NTU) | inf. à 5 | 5 à 10 | 10 à 50 | sup. à 50 |')
    st.markdown('| Température (°C) | inf. à 25 | 25 à 30 | 30 à 35 | sup. à 35 |')
    st.markdown('| Conductivité (µS/cm) | inf. à 2 500 | 2 500 à 3 000 | 3 000 à 4 000 | sup. à 4 000 |')
    st.markdown('| Nitrates (mg/L) | inf. à 50 | 50 à 80 | 80 à 150 | sup. à 150 |')
    st.markdown('| O₂ dissous (mg/L) | sup. à 6 | 4 à 6 | 2 à 4 | inf. à 2 |')
    st.markdown('| E. coli (UFC/100 mL) | 0 | 1 à 10 | 10 à 500 | sup. à 500 |')

with st.expander('🔬 Protocole de mesure — 7 paramètres'):
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>🧪 pH — Potentiomètre / pH-mètre</span><span class=\'proto-item\'>🔧 Outil : pH-mètre numérique ou bandelettes indicatrices de pH</span><span class=\'proto-item\'>1. Étalonner avec les solutions tampon pH 4, pH 7 et pH 10</span><span class=\'proto-item\'>2. Plonger l’électrode propre dans l’échantillon</span><span class=\'proto-item\'>3. Attendre la stabilisation de la valeur (environ 30 secondes)</span><span class=\'proto-item\'>4. Lire et noter la valeur affichée sur l’écran</span><span class=\'proto-item\'>5. Rincer l’électrode à l’eau distillée après chaque mesure</span></div>', unsafe_allow_html=True)
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>🌊 Turbidité — Turbidimètre numérique (NTU)</span><span class=\'proto-item\'>🔧 Outil : Turbidimètre numérique ou comparateur visuel</span><span class=\'proto-item\'>1. Remplir le tube de mesure avec l’échantillon</span><span class=\'proto-item\'>2. Essuyer le tube pour éliminer toute trace de doigts</span><span class=\'proto-item\'>3. Insérer le tube dans le turbidimètre et fermer le couvercle</span><span class=\'proto-item\'>4. Lire la valeur en NTU affichée</span><span class=\'proto-item\'>5. Répéter 3 fois et calculer la moyenne</span></div>', unsafe_allow_html=True)
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>🌡️ Température — Thermomètre électronique</span><span class=\'proto-item\'>🔧 Outil : Thermomètre numérique à sonde immergeàble (précision 0,1 °C)</span><span class=\'proto-item\'>1. S'assurer que la sonde est propre et sèche avant usage</span><span class=\'proto-item\'>2. Plonger la sonde directement dans l’échantillon d’eau</span><span class=\'proto-item\'>3. Attendre la stabilisation de la valeur (1 à 2 minutes)</span><span class=\'proto-item\'>4. Lire et noter la température en °C affichée</span><span class=\'proto-item\'>5. Mesurer idéalement sur le terrain, à la source du prélèvement</span></div>', unsafe_allow_html=True')
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>⚡ Conductivité — Conductimètre numérique (µS/cm)</span><span class=\'proto-item\'>🔧 Outil : Conductimètre portable avec cellule de conductivité</span><span class=\'proto-item\'>1. Étalonner l’appareil avec une solution étalon certifiée</span><span class=\'proto-item\'>2. Rincer la cellule deux fois avec l’échantillon à analyser</span><span class=\'proto-item\'>3. Plonger la cellule dans l’échantillon et activer la mesure</span><span class=\'proto-item\'>4. Attendre la stabilisation et lire la valeur en µS/cm</span><span class=\'proto-item\'>5. Rincer la cellule à l’eau distillée après usage</span></div>', unsafe_allow_html=True)
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>🌿 Nitrates — Spectrophotomètre ou bandelettes réactives</span><span class=\'proto-item\'>🔧 Outil : Spectrophotomètre portable ou kit colorimétrique (bandelettes nitrates)</span><span class=\'proto-item\'>1. Avec spectrophotomètre : filtrer l’échantillon, régler à 220 nm, étalonner avec eau distillée</span><span class=\'proto-item\'>2. Avec bandelettes : tremper la bandelette 1 seconde dans l’eau</span><span class=\'proto-item\'>3. Attendre le temps de réaction indiqué (généralement 60 secondes)</span><span class=\'proto-item\'>4. Comparer la couleur obtenue à la charte colorimétrique fournie</span><span class=\'proto-item\'>5. Lire et noter la concentration en mg/L</span></div>', unsafe_allow_html=True)
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>💨 Oxygène dissous — Oxymètre électronique</span><span class=\'proto-item\'>🔧 Outil : Oxymètre numérique portable avec sonde à membrane</span><span class=\'proto-item\'>1. Étalonner la sonde dans l’air saturé en humidité (10 min)</span><span class=\'proto-item\'>2. Plonger la sonde dans l’échantillon sans créer de bulles d’air</span><span class=\'proto-item\'>3. Agiter très doucement pour homogénéiser l’échantillon</span><span class=\'proto-item\'>4. Attendre 1 à 2 minutes et lire la valeur en mg/L</span><span class=\'proto-item\'>5. Rincer la sonde à l’eau distillée après usage</span></div>', unsafe_allow_html=True)
    st.markdown('<div class=\'proto-box\'><span class=\'proto-title\'>🧫 E. coli — Test de présence / absence ou dénombrement</span><span class=\'proto-item\'>🔧 Outil : Kit Colilert (IDEXX), bandelettes Compact Dry EC ou milieu m-FC en laboratoire</span><span class=\'proto-item\'>1. Prélever 100 mL d’eau dans un flacon stérile</span><span class=\'proto-item\'>2. Ajouter le réactif Colilert et mélanger jusqu’à dissolution complète</span><span class=\'proto-item\'>3. Incuber à 35 °C pendant 24 à 28 heures dans un incubateur</span><span class=\'proto-item\'>4. Lire le résultat : fluorescence sous UV = présence d’E. coli</span><span class=\'proto-item\'>5. Exprimer le résultat en UFC/100 mL et noter immédiatement</span></div>', unsafe_allow_html=True)
...
st.markdown('<div class=\'pcard\'><span class=\'plabel\'>🧪 pH — Potentiel Hydrogène</span><span class=\'ptext\'>Mesure l’acidité ou la basicité de l’eau. pH bas : métaux toxiques. pH élevé : contamination chimique.</span><span class=\'pnorm\'>Norme OMS : 6,5 à 8,5</span></div>', unsafe_allow_html=True)
...
st.markdown('<div class=\'pcard\'><span class=\'plabel\'>🌊 Turbidité (NTU) — Trouble de l’eau</span><span class=\'ptext\'>Particules en suspension. Eau trouble : agents pathogènes possibles, désinfection moins efficace.</span><span class=\'pnorm\'>Norme OMS : inférieur à 5 NTU</span></div>', unsafe_allow_html=True)
c4,c5,c6=st.columns(3)
tu1=c4.number_input('Turbidité — Mesure 1',0.0,200.0,2.0,0.01); tu2=c5.number_input('Turbidité — Mesure 2',0.0,200.0,2.0,0.01); tu3=c6.number_input('Turbidité — Mesure 3',0.0,200.0,2.0,0.01)
tu=round((tu1+tu2+tu3)/3,3); st.caption(f'📊 Moyenne Turbidité : {tu:.3f} NTU')

st.markdown('<div class=\'pcard\'><span class=\'plabel\'>🌡️ Température (°C) — Activité microbienne</span><span class=\'ptext\'>Au-delà de 25 °C, la prolifération des micro-organismes pathogènes s’accélère de manière significative.</span><span class=\'pnorm\'>Norme OMS : inférieur à 25 °C</span></div>', unsafe_allow_html=True)
c7,c8,c9=st.columns(3)
te1=c7.number_input('Temp. — Mesure 1 (°C)',0.0,60.0,22.0,0.1); te2=c8.number_input('Temp. — Mesure 2 (°C)',0.0,60.0,22.0,0.1); te3=c9.number_input('Temp. — Mesure 3 (°C)',0.0,60.0,22.0,0.1)
te=round((te1+te2+te3)/3,2); st.caption(f'📊 Moyenne Température : {te:.2f} °C')

st.markdown('<div class=\'pcard\'><span class=\'plabel\'>⚡ Conductivité (µS/cm) — Minéralisation de l’eau</span><span class=\'ptext\'>Une conductivité élevée indique une concentration excessive en sels et minéraux dissous, pouvant nuire à la santé.</span><span class=\'pnorm\'>Norme OMS : inférieur à 2 500 µS/cm</span></div>', unsafe_allow_html=True)
...
co=round((co1+co2+co3)/3,1); st.caption(f'📊 Moyenne Conductivité : {co:.1f} µS/cm')

st.markdown('<div class=\'pcard\'><span class=\'plabel\'>🌿 Nitrates (mg/L) — Pollution agricole</span><span class=\'ptext\'>Proviennent des engrais agricoles. Au-delà de 50 mg/L, ils provoquent la méthémoglobinémie chez les nourrissons.</span><span class=\'pnorm\'>Norme OMS : inférieur à 50 mg/L</span></div>', unsafe_allow_html=True)
c13,c14,c15=st.columns(3)
no1=c13.number_input('NO₃ — Mesure 1',0.0,500.0,5.0,0.1); no2=c14.number_input('NO₃ — Mesure 2',0.0,500.0,5.0,0.1); no3=c15.number_input('NO₃ — Mesure 3',0.0,500.0,5.0,0.1)
no=round((no1+no2+no3)/3,3); st.caption(f'📊 Moyenne Nitrates : {no:.3f} mg/L')

st.markdown('<div class=\'pcard\'><span class=\'plabel\'>💨 Oxygène dissous (mg/L) — Vitalité de l’eau</span><span class=\'ptext\'>Taux faible : décomposition organique intense, bactéries. En dessous de 2 mg/L : eau anoxique et dangereuse.</span><span class=\'pnorm\'>Norme : supérieur à 6 mg/L</span></div>', unsafe_allow_html=True)
c16,c17,c18=st.columns(3)
o1=c16.number_input('O₂ — Mesure 1',0.0,14.0,7.0,0.01); o2_2=c17.number_input('O₂ — Mesure 2',0.0,14.0,7.0,0.01); o3=c18.number_input('O₂ — Mesure 3',0.0,14.0,7.0,0.01)
o2=round((o1+o2_2+o3)/3,3); st.caption(f'📊 Moyenne O₂ dissous : {o2:.3f} mg/L')

st.markdown('<div class=\'pcard\'><span class=\'plabel\'>🧫 E. coli (UFC/100 mL) — Contamination fécale</span><span class=\'ptext\'>Indicateur direct de contamination fécale. Toute présence signale un risque sanitaire et la probable présence d’autres pathogènes.</span><span class=\'pnorm\'>Norme OMS : 0 UFC/100 mL (eau potable)</span></div>', unsafe_allow_html=True)
...
st.markdown('---')
MP={0:('💧 POTABLE','potable','Eau conforme aux normes OMS. Consommation possible sans risque.'),1:('⚠️ DOUTEUSE','douteuse','Anomalies détectées. Filtrez et faites bouillir avant consommation.'),2:('❌ POLLUÉE','polluee','Eau polluée. Ne pas consommer. Traitement obligatoire.'),3:('☠️ DANGEREUSE','dangereuse','DANGER EXTRÊME. Tout contact à éviter. Risque sanitaire majeur.'),}
    ...
    if erreurs:
        ...
        if cl in [1,2,3]:
            with st.expander('🛠️ Comment purifier cette eau ?'):
                ...
                st.markdown('<div class=\'conseil-box\'><span class=\'conseil-title\'>🧴 2. Filtration sur sable et gravier</span><span class=\'conseil-item\'>Couches : gravier grossier, fin, sable grossier, fin, charbon de bois actif.</span><span class=\'conseil-item\'>Compléter obligatoirement avec l’ébullition.</span></div>', unsafe_allow_html=True)
                st.markdown('<div class=\'conseil-box\'><span class=\'conseil-title\'>☀️ 3. Désinfection solaire SODIS</span><span class=\'conseil-item\'>Bouteilles transparentes : 6 heures au soleil (ciel clair) ou 2 jours (nuageux).</span><span class=\'conseil-item\'>Méthode gratuite, validée par l’OMS pour l’Afrique de l’Ouest.</span></div>', unsafe_allow_html=True)
                st.markdown('<div class=\'conseil-box\'><span class=\'conseil-title\'>🧪 4. Chloration</span><span class=\'conseil-item\'>2 gouttes d’eau de Javel à 5 % par litre (1 si eau claire). Attendre 30 min.</span></div>', unsafe_allow_html=True)
                st.markdown('<div class=\'conseil-box\'><span class=\'conseil-title\'>🌱 5. Graines de Moringa oleifera</span><span class=\'conseil-item\'>Broyer 2-3 graines sèches. Ajouter à 1 litre d’eau turbide, agiter 1 min puis 5 min lentement. Décanter 1 heure.</span></div>', unsafe_allow_html=True)
        st.markdown('---')
        st.markdown('### 📄 Rapport officiel PDF')
        try:
            mesures={'pH':pH,'turb':tu,'temp':te,'cond':co,'no3':no,'o2':o2,'ecoli':ec}
            pdf_bytes=generer_pdf(mesures,cl,list(pr),analyste=analyste,lieu=lieu,source=source)
        ...
        st.session_state.histo.append({'Heure':datetime.now().strftime('%H:%M:%S'),'Analyste':analyste,'Lieu':lieu,'Source':source,'pH':pH,'Turb.':tu,'Temp.':te,'Cond.':co,'NO3':no,'O2':o2,'E.coli':ec,'Résultat':lb})
...
st.markdown('---')
st.markdown('<div style=\'text-align:center;color:#023e8a !important;font-size:12px;padding:10px;font-weight:600;\'>💧 EauVie — Random Forest — Normes OMS — 7 paramètres — Charles MEDEZOUNDJI</div>', unsafe_allow_html=True)
