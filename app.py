...
def evaluer_sous_reserve(vals, cl):
    ...
    labels = ["POTABLE", "DOUTEUSE", "POLLUEE", "DANGEREUSE"]
    label = labels[cl]
    if sous_reserve:
...
CONTEXTES_PDF = {
    "potable": {
        "chiffres": (
            "Plus de 400 millions d'Africains n'ont pas acces a une eau potable "
            "sure (OMS/UNICEF, 2025). Au Benin, les maladies d'origine hydrique "
            "- diarrhee, cholera, fievre typhoide - representent plus de 70 % des "
            "maladies infantiles. Chaque annee, des milliers d'enfants de moins de "
            "cinq ans meurent de maladies directement liees a la consommation d'une "
            "eau de mauvaise qualite. L'enquete Afrobarometre 2024 classe l'acces "
            "a l'eau potable comme le premier defi prioritaire des citoyens beninois, "
            "devant l'emploi et l'education."
        ),
        "problematique": (
            "La veritable problematique ne reside pas seulement dans le manque d'eau, "
            "mais dans l'absence d'outils accessibles pour evaluer sa qualite. "
            "Les laboratoires agrees sont rares, couteux et souvent eloignes des "
            "zones rurales. Une analyse complete peut couter plusieurs milliers de "
            "francs CFA et prendre plusieurs jours, rendant toute decision urgente "
            "impossible sur le terrain. Des millions de personnes consomment "
            "quotidiennement une eau dont elles ignorent la qualite reelle."
        ),
        "solution": (
            "EauVie repond a cette urgence en permettant a tout agent de sante, "
            "analyste ou technicien de realiser une analyse complete de la qualite "
            "de l'eau potable en quelques minutes, directement sur le terrain. "
            "L'application analyse 11 parametres classes en trois categories "
            "(microbiologie, physico-chimique, chimique) selon les normes OMS 2017 "
            "et la norme beninoise NB 001/2001, grâce a un algorithme Random Forest "
            "entraine sur 120 echantillons reels representatifs du Benin et de "
            "l'Afrique de l'Ouest, avec une precision validee a 100 %."
        ),
    },
    "usee": {
        "chiffres": (
            "En Afrique subsaharienne, plus de 90 % des eaux usees sont rejetees "
            "sans traitement dans le milieu naturel (PNUE, 2016). Au Benin, "
            "l'absence de stations d'epuration fonctionnelles dans la majorite "
            "des villes secondaires entraine une degradation continue des cours "
            "d'eau, des lagunes et des nappes phreatiques. A Cotonou, les etudes "
            "de Vodounnou et al. (2020) montrent que plus de 80 % des effluents "
            "industriels et artisanaux sont deverses directement dans le milieu "
            "recepteur sans traitement prealable, provoquant une contamination "
            "grave des ecosystemes aquatiques cotieres."
        ),
        "problematique": (
            "La gestion des eaux usees industrielles, domestiques et agricoles "
            "constitue un defi majeur pour la sante publique et l'environnement "
            "au Benin. L'absence de controle systematique des effluents avant "
            "rejet permet a des polluants dangereux - metaux lourds, composes "
            "organiques, agents pathogenes - de contaminer les milieux recepteurs. "
            "Les normes de rejet existent (arrete beninois 2001-294) mais leur "
            "application reste insuffisante faute d'outils de terrain accessibles "
            "et de personnels formes."
        ),
        "solution": (
            "EauVie integre un module dedie a l'analyse des eaux usees, permettant "
            "aux inspecteurs, agents environnementaux et industriels de verifier "
            "la conformite de leurs effluents avant rejet. L'analyse couvre "
            "8 parametres cles (pH, DBO5, DCO, MES, temperature, ammonium, plomb, "
            "E. coli) conformement a l'arrete beninois 2001-294, aux directives "
            "OMS 2006 et a la Directive europeenne 91/271/CEE. Le resultat est "
            "immediat et accompagne de recommandations de traitement adaptees."
        ),
    },
    "naturelle": {
        "chiffres": (
            "Les cours d'eau du Benin subissent une degradation acceleree sous "
            "l'effet conjugue de l'agriculture intensive, de l'urbanisation "
            "non maitrisee et du changement climatique. Les etudes d'Imorou Toko "
            "et al. (2010) sur le bassin de la Sota (Nord-Benin) et de Boukari "
            "et al. (2003) sur les eaux cotieres revelent une deterioration "
            "significative de la qualite ecologique de la majorite des masses "
            "d'eau superficielles. La Directive-Cadre sur l'Eau (DCE 2000/60/CE) "
            "fixe un objectif de bon etat ecologique pour toutes les masses d'eau "
            "d'ici 2027, objectif encore tres loin d'etre atteint en Afrique "
            "de l'Ouest."
        ),
        "problematique": (
            "Le suivi de la qualite ecologique des eaux naturelles (rivieres, lacs, "
            "marigots, zones humides) necessite des analyses regulieres de "
            "parametres physico-chimiques et biologiques. Or, au Benin, ce suivi "
            "reste tres insuffisant et concentre sur quelques stations "
            "hydrometriques de la Direction Nationale de l'Hydraulique. "
            "Les communautes riveraines, les agriculteurs et les gestionnaires "
            "de territoires n'ont acces a aucun outil simple pour evaluer l'etat "
            "de leurs masses d'eau locales, rendant impossible toute action "
            "preventive ou corrective en temps reel."
        ),
        "solution": (
            "Le module Eaux naturelles d'EauVie permet a tout acteur de terrain "
            "d'evaluer rapidement la qualite ecologique d'un cours d'eau, lac ou "
            "marigot en mesurant 7 parametres cles (pH, turbidite, oxygene dissous, "
            "nitrates, E. coli, temperature, conductivite) selon les referentiels "
            "de la DCE 2000/60/CE et de l'IBGN (NF T90-350). Chaque analyse "
            "contribue a la construction d'une cartographie nationale de la qualite "
            "des eaux naturelles, indispensable aux decisions des autorites et "
            "des organisations de gestion des bassins versants."
        ),
    },
    "agricole": {
        "chiffres": (
            "L'agriculture represente plus de 70 % des prelevements en eau douce "
            "en Afrique subsaharienne (FAO, 2020). Au Benin, les zones maraicheres "
            "periurbaines de Cotonou, Abomey-Calavi et Parakou utilisent "
            "massivement des eaux d'irrigation dont la qualite n'est pas evaluee. "
            "Les etudes de Mama et al. (2011) montrent que plus de 60 % des eaux "
            "utilisees pour l'irrigation dans ces zones presentent des taux de "
            "sodisation et de salinisation superieurs aux seuils FAO, provoquant "
            "une degradation progressive et irreversible des sols agricoles. "
            "Les pertes de rendement atteignent 30 a 50 % dans les zones affectees."
        ),
        "problematique": (
            "La qualite de l'eau d'irrigation est un facteur determinant pour "
            "la productivite agricole et la durabilite des sols. Une eau trop "
            "saline, trop sodique ou trop chargee en nitrates provoque a terme "
            "une salinisation des sols, une destruction de leur structure, une "
            "toxicite ionique pour les plantes et une contamination des nappes "
            "phreatiques. Au Benin, les agriculteurs n'ont generalement acces "
            "a aucune analyse de la qualite de leur eau d'irrigation, prenant "
            "des risques graves pour leurs sols et leurs cultures sans en avoir "
            "conscience."
        ),
        "solution": (
            "Le module Eau agricole d'EauVie permet aux agriculteurs, agronomes "
            "et conseillers agricoles d'evaluer instantanement l'aptitude d'une "
            "eau a l'irrigation selon les normes FAO 1994 (Ayers & Westcot). "
            "L'application analyse 7 parametres (conductivite, pH, sodium, "
            "nitrates, calcium, magnesium) et calcule automatiquement le SAR "
            "(rapport d'absorption du sodium), indicateur cle de la sodicite. "
            "Le resultat oriente immediatement l'agriculteur vers les amendements "
            "et pratiques culturales adaptes pour preserver la fertilite de ses sols."
        ),
    },
}
...
def construire_pdf(titre_mod, sous_titre, couleur_mod, params_aff,
    ...
    for i, n in enumerate([
        "Ce rapport est genere automatiquement par EauVie. La decision est fondee "
        "sur les parametres disponibles. La fiabilite depend directement de la "
        "precision des mesures effectuees sur le terrain.",
        ...
        "Ce rapport ne se substitue pas a une analyse complete en laboratoire agree. "
        "Pour une certification officielle de conformite, des tests complementaires "
        "en laboratoire sont recommandes.",
    ], 1):
        story.append(Paragraph(
            "<b>Note " + str(i) + " :</b> " + n, sn))
    ...
    ft = Table(
        [[
            ...
            Paragraph(
                ...
                "Ne remplace pas un labo agree<br/>"
                "<b>EauVie 2026</b>",
...
def triple(cle, label, desc, norme, mn, mx, defv, step, unite="", optionnel=False):
    ...
    if optionnel:
        ...
        pas_m = st.checkbox(f"Pas mesure - {label.split('-')[0].strip()}", key=key_pm)
        if pas_m:
            st.caption("Parametre non mesure - indique dans le rapport.")
            return None
...
def bloc_analyste(sources_list):
    st.markdown('<span class="section-title">Analyste et prelevement</span>',
                unsafe_allow_html=True)
    a = st.text_input("Nom complet de l'analyste *",
                       placeholder="Ex : Jean KOFFI", key=f"analyste_{module}")
    l = st.text_input("Lieu de prelevement *",
...
def afficher_resultat(label_final, cs, conf, pr, classes_labels,
    ...
    if sous_reserve and params_manquants:
        st.markdown(
            ...
            f'Decision provisoire - ajoutez ces mesures pour confirmation.'
            f'</div>',
            unsafe_allow_html=True,
        )
    st.markdown(f"**Conseil :** {conseil_txt}")
    if methodes and classe_pred > 0:
        with st.expander("Recommandations detaillees"):
            for m, d in methodes:
                st.markdown(
...
def bloc_pdf_et_carto(r, key_pdf, key_carto, module_label):
    ...
    st.markdown('<div class="carto-box">Cartographie - Si mesure reelle, ajoutez-la a la carte communautaire.</div>',
                unsafe_allow_html=True)
    if st.button("Ajouter a la cartographie", key=key_carto):
        ...
        st.success(f"Ajoute ({len(st.session_state.carto_points)} point(s)).")
...
if module == "potable":
    st.markdown(
        ...
        ' afin de garantir une consommation rassurante et benefique</div>'
        '<div class="header-author">Charles MEDEZOUNDJI - Normes OMS - Benin</div>'
        '</div>',
        unsafe_allow_html=True,
    )
...
<tr><td>Chlore residuel (mg/L)</td><td>0,2-0,5</td><td>0,05-0,2</td><td>&lt;0,05</td><td>0</td></tr>
    ...
    st.markdown('<span class="section-title">Saisie des 11 parametres - 3 categories</span>',
                unsafe_allow_html=True)
    st.info("Saisissez 3 mesures par parametre. Cochez 'Pas mesure' si non disponible.")

    st.markdown('<div class="cat-box cat-micro">'
                '<span class="cat-title cat-title-micro">Qualite microbiologique</span>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="cat-box cat-physico">'
                '<span class="cat-title cat-title-physico">Qualite physico-chimique</span>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="cat-box cat-chimique">'
                '<span class="cat-title cat-title-chimique">Qualite chimique</span>',
                unsafe_allow_html=True)
    ...
    st.markdown("---")
    if st.button("Analyser la qualite de l'eau potable", key="btn_potable"):
        erreurs = []
        ...
        if erreurs:
            ...
            conseils = [
                ...
                "Anomalies detectees. Filtrez et faites bouillir avant consommation.",
                "Eau polluee. Traitement complet obligatoire avant tout usage.",
                "DANGER EXTREME. Tout contact a eviter. Signalez aux autorites.",
            ]
            methodes_p = [
                ("Ebullition", "5 min minimum. Efficace contre bacteries, virus, parasites."),
                ("Filtration sable/gravier", "Gravier + sable + charbon actif. Completer par ebullition."),
            ...
            try:
                pdf = construire_pdf(
                    ...
                    ["POTABLE", "DOUTEUSE", "POLLUEE", "DANGEREUSE"], cl_pred,
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        '<div class="header-title">EauVie - Eaux Usees & Industrielles</div>'
        ...
        ' - Arrete Benin 2001-294 - OMS 2006 - Directive 91/271/CEE</div>'
        '</div>',
        unsafe_allow_html=True,
    )

    LABELS_EU = ["CONFORME AU REJET", "LIMITE", "NON CONFORME", "TRES POLLUEE"]
...
<tr><td>E. coli (UFC/100 mL)</td><td>&lt;2 000</td><td>2 000-10 000</td>
    ...
    st.markdown('<span class="section-title">Parametres des eaux usees</span>',
                unsafe_allow_html=True)

    st.markdown('<div class="cat-box cat-dbo">'
                '<span class="cat-title cat-title-dbo">Parametres organiques et physiques</span>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="cat-box cat-chimique">'
                '<span class="cat-title cat-title-chimique">Parametres chimiques et microbiologiques</span>',
                unsafe_allow_html=True)
    ...
    st.markdown("---")
    if st.button("Analyser les eaux usees", key="btn_eu"):
        erreurs = []
        if not analyste.strip():
            erreurs.append("Nom de l'analyste obligatoire.")
        if not lieu.strip():
        ...
        if erreurs:
            ...
            methodes_eu = [
                ("Traitement primaire",
                 "Decantation, flottation : elimination des MES. Reduction DBO 30-40 %."),
                ("Traitement secondaire biologique",
                 "Boues activees ou lagunage : degradation matiere organique. Reduction DBO > 85 %."),
                ("Traitement tertiaire",
                 "Desinfection UV ou chloration pour eliminer les pathogenes avant rejet."),
                ("Neutralisation pH",
                 "Ajout de chaux (si pH acide) ou acide chlorhydrique (si pH alcalin)."),
                ("Traitement metaux lourds",
                 "Precipitation chimique, echange d'ions ou electrolyse pour le Plomb."),
            ]
            st.session_state.analyse_faite = True
            st.session_state.dernier_resultat = {
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        '<div class="header-title">EauVie - Eaux Naturelles & Ecologiques</div>'
        '<div class="header-sub">Analyse intelligente des eaux naturelles'
        ' pour une ecologie stable et durable</div>'
        '<div class="header-author">Charles MEDEZOUNDJI'
        ' - DCE 2000/60/CE - IBGN - Imorou Toko 2010</div>'
        '</div>',
        unsafe_allow_html=True,
    )

    LABELS_EN = ["BONNE QUALITE", "QUALITE MOYENNE", "MAUVAISE QUALITE", "TRES MAUVAISE QUALITE"]
    CSS_EN = ["bonne", "moyenne", "mauvaise", "tres-mauvaise"]
    CONSEILS_EN = [
        "Eau de bonne qualite ecologique. Le milieu aquatique est en bon etat. Faune et flore preservees.",
        "Qualite moyenne. Des pressions moderees affectent ce milieu. Actions preventives recommandees.",
        "Mauvaise qualite. Ce milieu est fortement perturbe. Mesures de restauration necessaires.",
        "TRES MAUVAISE QUALITE. Milieu aquatique gravement degrade. Intervention urgente requise.",
    ]
...
<tr><td>Conductivite (uS/cm)</td><td>&lt;400</td><td>400-1 000</td>
    ...
    st.markdown('<span class="section-title">Parametres ecologiques</span>',
                unsafe_allow_html=True)
    st.markdown('<div class="cat-box cat-ecologie">'
                '<span class="cat-title cat-title-eco">Qualite physico-chimique et biologique</span>',
                unsafe_allow_html=True)
    ...
    st.markdown("---")
    if st.button("Analyser la qualite ecologique", key="btn_en"):
        erreurs = []
        if not analyste.strip():
            erreurs.append("Nom de l'analyste obligatoire.")
        if not lieu.strip():
        ...
        if erreurs:
            ...
            methodes_en = [
                ("Reduction intrants agricoles",
                 "Limiter les engrais azotes. Zones tampons vegetalisees en bordure de cours d'eau."),
                ("Traitement des rejets urbains",
                 "Mettre en place des stations d'epuration en amont. Traitement obligatoire avant rejet."),
                ("Restauration hydromorphologique",
                 "Remeandrement, recharge granulometrique, plantation de ripisylve."),
                ("Reduction pollutions diffuses",
                 "Sensibilisation agricole. Reduction pesticides. Gestion des dechets."),
                ("Surveillance reguliere",
                 "Mise en place d'un reseau de surveillance avec analyses periodiques."),
            ]
            st.session_state.analyse_faite = True
            st.session_state.dernier_resultat = {
            ...
            try:
                pdf = construire_pdf(
                    "Eaux Naturelles & Ecologiques",
                    "Analyse pour une ecologie stable et durable",
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        '<div class="header-title">EauVie - Eau Agricole & Irrigation</div>'
        '<div class="header-sub">Analyse intelligente de l\'eau agricole'
        ' pour une agriculture rentable et saine</div>'
        '<div class="header-author">Charles MEDEZOUNDJI'
        ' - Normes FAO 1994 - SODAGRI Benin - Mama 2011</div>'
        '</div>',
        unsafe_allow_html=True,
    )

    LABELS_EA = ["BONNE APTITUDE", "APTITUDE MODEREE", "EAU A RISQUE", "EAU INADAPTEE"]
...
<tr><td>SAR</td><td>&lt;3</td><td>3-9</td>
    ...
    st.markdown('<span class="section-title">Parametres qualite eau agricole</span>',
                unsafe_allow_html=True)
    st.markdown('<div class="cat-box cat-agri">'
                '<span class="cat-title cat-title-agri">Parametres FAO - Salinite, sodicite, nutrition</span>',
                unsafe_allow_html=True)
    ...
    sar_lbl = (
        "Bonne aptitude (<3)" if sar_calc < 3
        else ("Modere (3-9)" if sar_calc < 9
              else ("Risque (9-18)" if sar_calc < 18 else "Inadapte (>18)"))
    )
    st.markdown(
    ...
    st.markdown("---")
    if st.button("Analyser l'aptitude a l'irrigation", key="btn_ea"):
        erreurs = []
        if not analyste.strip():
            erreurs.append("Nom de l'analyste obligatoire.")
        if not lieu.strip():
        ...
        if erreurs:
            ...
            methodes_ea = [
                ("Gypse (CaSO4)",
                 "Apport de gypse pour corriger la sodicite. Dose : 2-5 t/ha selon SAR."),
                ("Drainage",
                 "Installer un drainage pour lessiver l'exces de sel. Eviter la remontee capillaire."),
                ("Cultures tolerantes",
                 "Choisir coton, sorgho, orge (tolerance elevee). Eviter haricot, mais."),
                ("Amendement calcique",
                 "Apport de chaux ou cendre de bois pour corriger le pH."),
                ("Eau de dilution",
                 "Melanger avec de l'eau de meilleure qualite pour reduire la conductivite."),
                ("Suivi ionique du sol",
                 "Analyses regulieres du sol (CE, SAR, pH). Minimum 2 fois par saison."),
            ]
            st.session_state.analyse_faite = True
            st.session_state.dernier_resultat = {
...
# ══════════════════════════════════════════════════════════════
st.markdown("---")
st.markdown('<span class="section-title">Cartographie des analyses - tous modules</span>',
            unsafe_allow_html=True)
mdp = st.text_input(
    "Mot de passe cartographie",
    type="password",
    placeholder="Saisir CARTOGRAPHIE",
    key="mdp_carto",
)
if mdp:
    if mdp == "CARTOGRAPHIE":
        ...
        nb = len(pts)
        st.success(f"Acces autorise. {nb} point(s) enregistre(s).")

        if nb == 0:
            st.info(
                "Aucune mesure ajoutee. Effectuez une analyse et cliquez"
                " sur 'Ajouter a la cartographie'."
            )
        else:
            ...
            st.info(
                f"{len(pts_filtres)} point(s) affiche(s)"
...
st.markdown(
    ...
    'Charles MEDEZOUNDJI - Developpeur EauVie</div>'
