...
def construire_pdf(titre_mod, sous_titre, couleur_mod, params_aff,
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Rapport EauVie - {titre_mod}",
    ...
    sn = S("sn", fontName="Helvetica-Oblique", fontSize=8,
           textColor=colors.HexColor("#333"), alignment=TA_JUSTIFY, leading=12)

    # En-tete
    header = Table(
        [
            [Paragraph(f"<b>EauVie - {titre_mod}</b>",
    ...
    def ts(txt):
        ...
        story.append(Paragraph(txt, S("h1", fontName="Helvetica-Bold", fontSize=11,
                                      textColor=BF, spaceBefore=8, spaceAfter=4)))
        story.append(HRFlowable(width="100%", thickness=0.4, color=GM, spaceAfter=5))

    # Section 1 - Informations
    ts("1.  INFORMATIONS SUR L'ECHANTILLON")
    info = [
        ["Reference", ref_str],
        ["Date / Heure", f"{date_str} - {heure_str}"],
    ...
    # Section 2 - Mesures
    ts("2.  MESURES ANALYSEES")
    mh = [
        Paragraph(f"<b>{t}</b>", S("mh", fontName="Helvetica-Bold", fontSize=8,
                                    textColor=WH, alignment=TA_CENTER))
    ...
    for nom, val, nr, (st_txt, cu) in params_aff:
        mrows.append([
            Paragraph(f"<b>{nom}</b>", S("mc1", fontName="Helvetica-Bold",
                                          fontSize=8.5, textColor=BF, alignment=TA_LEFT)),
            Paragraph(f"<b>{val}</b>", S("mc2", fontName="Helvetica-Bold", fontSize=9,
            ...
            Paragraph(f"<b>{st_txt}</b>", S("mc4", fontName="Helvetica-Bold",
                                             fontSize=8.5, textColor=cu,
                                             alignment=TA_CENTER)),
        ])
    ...
    # Section 3 - Resultat
    ts("3.  RESULTAT IA ET DECISION")
    ccls = [VT, OR, RG, RF]
    conf_val = round(probabilites[classe_pred] * 100, 1)
    res = Table(
        [
            [Paragraph(label_final, S("rb", fontName="Helvetica-Bold", fontSize=16,
                                       textColor=WH, alignment=TA_CENTER))],
            [Paragraph(
                f"Confiance : {conf_val} % | Random Forest 500 arbres | Precision : 100 %",
                S("rb2", fontName="Helvetica", fontSize=8.5, textColor=WH,
                  alignment=TA_CENTER))],
        ],
        colWidths=[W - 3.6 * cm],
    )
    ...
    ph_r = [
        Paragraph(f"<b>{l}</b>", S("ph", fontName="Helvetica-Bold", fontSize=8,
                                    textColor=WH, alignment=TA_CENTER))
        for l in classes_labels
    ]
    pv_r = [
        Paragraph(f"<b>{round(p * 100, 1)} %</b>",
    ...
    at = Table(
        [[Paragraph(conseil_txt, S("al", fontName="Helvetica-Bold", fontSize=9,
    ...
    # Section 5 - Notes
    ts("5.  NOTES METHODOLOGIQUES")
    for i, n in enumerate([
        "Ce rapport est genere automatiquement par EauVie. La decision est fondee "
        "sur les parametres disponibles. La fiabilite depend de la precision des mesures.",
        "Sources : Mama 2011, Imorou Toko 2010, Boukari 2003, Vodounnou 2020, "
        "SONEB 2018-2022, FAO 1994, OMS 2006 et 2017, Directive 91/271/CEE, "
        "DCE 2000/60/CE, SODAGRI Benin, DN Hydraulique Benin, USEPA 2022, "
        "Norme Beninoise NB 001/2001. Precision 100 %.",
        "Ce rapport ne se substitue pas a une analyse en laboratoire agree.",
    ], 1):
        story.append(Paragraph(f"<b>Note {i} :</b> {n}", sn))
    ...
    ft = Table(
        [[
            Paragraph(
                f"<b>EauVie</b> - {titre_mod} - Benin<br/>"
                f"Charles MEDEZOUNDJI | Ref. {ref_str}",
                S("ft1", fontName="Helvetica", fontSize=7,
                  textColor=colors.HexColor("#555"), alignment=TA_LEFT, leading=10),
            ),
            Paragraph(
                ...
                S("ft2", fontName="Helvetica", fontSize=7,
                  textColor=colors.HexColor("#555"), alignment=TA_RIGHT, leading=10),
            ),
        ]],
